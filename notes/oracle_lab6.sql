...
SPOOL /home/student/oracle_lab6.txt replace

/*IMPLEMENT the verify_date function.*/
CREATE OR REPLACE
...
/*TEST the implementation of the verify_date function.*/
DECLARE
  -- Test set values.
...
/*Create a list data type of 100 character variable length strings with the name tre*/
CREATE OR REPLACE TYPE tre AS TABLE OF VARCHAR2(100);
/
...
/*
Create cast_strings function that accepts a single parameter of the tre, a Attribute Data Type (ADT) created above; and returns a struct structure as qualified above.
HINT #1: Conditional compilation is available in PL/SQL and you should enable it to debug your cast_strings function, as discussed in Oracle Database 12c PL/SQL Programming, Chapter 8[pp. 169-171].
HINT #2: Check and understand how POSIX regular expressions are implemented in the Oracle database by reviewing Oracle Database 12c PL/SQL Programming, Appendix E [pp. 1000-1017].
*/
CREATE OR REPLACE
  ...
  ( pv_list  TRE ) RETURN struct IS
    ...
    lv_retval  STRUCT := struct(NULL, NULL, NULL); 
    ...
    FOR i IN 1..pv_list.LAST LOOP
      ...
      IF pv_list.EXISTS(i) THEN
        ...
        CASE
          ...
          WHEN lv_retval.xnumber IS NULL AND REGEXP_LIKE(pv_list(i), '^\d+$') THEN
          ...
          WHEN lv_retval.xdate IS NULL AND TO_DATE(pv_list(i), 'DD-MON-YY') IS NOT NULL THEN
          lv_retval.xdate := TO_DATE(pv_list(i), 'DD-MON-YY');
...
CREATE OR REPLACE TYPE structs
IS TABLE OF struct;
/
...
SELECT TO_CHAR(xdate,'DD-MON-YYYY') AS 
    xdate
,      xnumber
,      xstring
